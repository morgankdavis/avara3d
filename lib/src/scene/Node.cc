...
vec3 Node::worldScale() const {
    ...
    return {length(vec3(w.c0)), length(vec3(w.c1)), length(vec3(w.c2))};
}
...
void Node::_debugPrintRec(Node& node, unsigned level) {
...
void Node::childrenRec(const shared_ptr<Node>& node, vector<shared_ptr<Node>>& children) const {
