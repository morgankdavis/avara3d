...
std::optional<VertexStreamView> VertexAccess::GetStreamView(const MeshElement& element,
    ...
    if (element.vertexStride() != d.stride) {
        return std::nullopt;
    }

    const std::size_t expectedSize = std::size_t(element.vertexCount()) * std::size_t(d.stride);

    A3D_ASSERT(vb.size() == expectedSize);
    if (vb.size() != expectedSize) {
        return std::nullopt;
    }

    const VertexAttribDesc* a = FindAttrib(d, semantic);
    if (!a || a->format != expectedFormat) {
        return std::nullopt;
    }
