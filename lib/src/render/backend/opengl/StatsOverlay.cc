...
namespace {
    ...
    constexpr std::chrono::milliseconds FRAME_STATS_AVERAGING_DURATION {250};

    // how often to recompute the frame stats
    constexpr std::chrono::milliseconds FRAME_STATS_AVERAGE_UPDATE_INTERVAL {100};
    ...
    void DrawDebugOptions(Scene& scene, ImFont& bodyFont); // TODO: remove?
...
namespace {
    ...
    void DrawStats(FrameStats&              stats,
        ...
        auto frameValue = std::format("{:.2f}ms", frameMsFAvg);
        ...
        ImguiDrawPlot(X_POS, yPos, COLUMN_WIDTH, PLOT_HEIGHT_1, frameSamples.data(),
                      static_cast<int>(frameSamples.size()), 0, nullptr, nullptr, 0.0f, PLOT_Y_MIN, PLOT_Y_MAX,
                      0, PLOT_OUTLINED, ++id, PLOT_HEIGHT_1 + PLOT_STR_Y_PAD, false);
        ...
        const auto discardedStepCount =
            discardedSteps && stats.simulationTimeStep > 0.0
                ? static_cast<std::uint64_t>(std::llround(stats.discardedSimulationTime
                                                          / stats.simulationTimeStep))
                : 0;
        ...
        const auto formatDiscardedTime = [](double seconds) {
            return seconds >= 1.0 ? std::format("{:.2f}s", seconds) : std::format("{:.2f}ms", seconds * 1000.0);
        };
        ...
        ImguiDrawLabelValue(yPos, layout, "step #",
                            std::format(numberLocale, "{:L}", stats.simulationStepCount), bodyFont,
        ...
        ImguiDrawLabelValue(yPos, layout, "draw",
                            (gpuTimingAvailable ? std::format("{:.2f}ms", renderGpuMsFAvg) : ""), bodyFont,
                            STATS_BODY_FONT_SIZE, PLOT_Y_PAD,
        ...
        ImguiDrawPlot(X_POS, yPos, COLUMN_WIDTH, PLOT_HEIGHT_2, renderGpuSamples.data(),
                      static_cast<int>(renderGpuSamples.size()), 0,
                      gpuTimingAvailable ? nullptr : "unavailable", &bodyFont, STATS_BODY_FONT_SIZE, PLOT_Y_MIN,
                      PLOT_Y_MAX, 0, PLOT_OUTLINED, ++id, PLOT_HEIGHT_2 + PLOT_STR_Y_PAD, !gpuTimingAvailable);

        ImguiDrawLabelValue(yPos, layout, "physics", std::format("{:.2f}ms", physicsMsFAvg), bodyFont,
                            STATS_BODY_FONT_SIZE, PLOT_Y_PAD);
        ImguiDrawPlot(X_POS, yPos, COLUMN_WIDTH, PLOT_HEIGHT_2, physSamples.data(),
                      static_cast<int>(physSamples.size()), 0, nullptr, nullptr, 0.0f, PLOT_Y_MIN, PLOT_Y_MAX,
                      0, PLOT_OUTLINED, ++id, PLOT_HEIGHT_2 + PLOT_STR_Y_PAD, false);

        ImguiDrawLabelValue(yPos, layout, "application", std::format("{:.2f}ms", appCpuMsFAvg), bodyFont,
                            STATS_BODY_FONT_SIZE, PLOT_Y_PAD);
        ...
        //                     STATS_BODY_FONT_SIZE, STAT_LINE_STEP);
        ImguiDrawLabelValue(yPos, bulkLayout, "polygons",
                            std::format("{:.1f}k", float(stats.polygons) / 1000.0f), bodyFont,
                            STATS_BODY_FONT_SIZE, STAT_LINE_STEP);
        ...
        ImguiDrawLabelValueIndented(yPos, bulkLayout, "dynamic", std::format("{}", stats.dynamicBodies),
        ...
        ImguiDrawLabelValueIndented(yPos, bulkLayout, "primitive", std::format("{}", stats.primitiveShapes),
                                    bodyFont, STATS_BODY_FONT_SIZE, INDENT_WIDTH, STAT_LINE_STEP);
        ImguiDrawLabelValueIndented(yPos, bulkLayout, "bbox", std::format("{}", stats.boundingBoxShapes),
                                    bodyFont, STATS_BODY_FONT_SIZE, INDENT_WIDTH, STAT_LINE_STEP);
        ImguiDrawLabelValueIndented(yPos, bulkLayout, "convex", std::format("{}", stats.convexHullShapes),
                                    bodyFont, STATS_BODY_FONT_SIZE, INDENT_WIDTH, STAT_LINE_STEP);
        ImguiDrawLabelValueIndented(yPos, bulkLayout, "concave",
                                    std::format("{}", stats.concavePolyhedronShapes), bodyFont,
                                    STATS_BODY_FONT_SIZE, INDENT_WIDTH, STAT_LINE_STEP);
    ...
    void DrawDebugOptions(Scene& scene, ImFont& bodyFont) {
        ...
        if (ImguiDrawCheckbox(xPos, yPos, "mesh AABBs", meshAABBs, bodyFont, STATS_BODY_FONT_SIZE, false,
                              ++id)) {
            if (meshAABBs) {
        ...
        yPos += Y_PAD;
        bool physContacts =
            util::bitmask::contains(scene.debugOptions(), DebugOptions::ShowPhysicsContactPoints);
        if (ImguiDrawCheckbox(xPos, yPos, "physics contacts", physContacts, bodyFont, STATS_BODY_FONT_SIZE,
                              false, ++id)) {
            if (physContacts) {
    ...
    void ImguiDrawSimulationPlot(float        x,
        ...
        PlotLines(("##simulation_steps_shadow" + std::to_string(id)).c_str(), stepValues, valuesCount, 0,
                  nullptr, 0.0f, scaleMax, ImVec2(w, h), sizeof(float));
        PopStyleColor(2);
        PopStyleVar();
        ...
        PlotLines(("##simulation_steps" + std::to_string(id)).c_str(), stepValues, valuesCount, 0, nullptr,
    ...
    void ImguiDrawPlot(float        x,
        ...
        SetCursorScreenPos(ImVec2(x + 1, y + 1));
        PushStyleColor(ImGuiCol_PlotLines, ImVec4(0, 0, 0, 1));
        PlotLines(("##plot_s" + std::to_string(id)).c_str(), values, valuesCount, valuesOffset, nullptr,
                  scaleMin, scaleMax, ImVec2(w, h), plotStride);
        PopStyleColor();
        ...
        if (overlayText != nullptr && overlayText[0] != '\0') {
            ...
            drawList->AddText(font, overlayFontSize,
