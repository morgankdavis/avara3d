...
namespace a3d::utils::flow::detail {
    ...
    inline long long clamp_step(long long n) { return (n < 1) ? 1 : n; }

} // namespace a3d::utils::flow::detail

namespace a3d::utils::flow {
    ...
    inline bool edge_guard(Cond&& cond,
        ...
        const bool ok = static_cast<bool>(std::forward<Cond>(cond)); // supports shared_ptr, etc.
        auto& s = ::a3d::utils::flow::detail::state_for<State>(loc);

        if (ok) { s.latched = false; return true; }
    ...
    inline decltype(auto) once(Fn&& fn,
        ...
        if constexpr (std::is_void_v<R>) {
            struct State { std::once_flag once; };
            auto& s = ::a3d::utils::flow::detail::state_for<State>(loc);
            std::call_once(s.once, [&]{ std::invoke(std::forward<Fn>(fn)); });
            return;
        }
        else {
            using T = std::decay_t<R>;
            struct State { std::once_flag once; std::optional<T> value; };
            auto& s = ::a3d::utils::flow::detail::state_for<State>(loc);

            std::call_once(s.once, [&]{ s.value.emplace(std::invoke(std::forward<Fn>(fn))); });
    ...
    inline decltype(auto) once_else(ThenFn&& then_fn,
        ...
        if constexpr (std::is_void_v<R1>) {
            ...
            if (!i_ran_then) std::invoke(std::forward<ElseFn>(else_fn));
            return;
        }
        else {
            ...
            if (i_ran_then) return *first_value; // by value (T)
    ...
    inline decltype(auto) on(long long invocation,
        ...
        auto& s = ::a3d::utils::flow::detail::state_for<State>(loc, invocation);
    ...
    inline decltype(auto) on_else(long long invocation,
        ...
        if constexpr (std::is_void_v<R1>) {
            if (fire) std::invoke(std::forward<ThenFn>(then_fn));
            else      std::invoke(std::forward<ElseFn>(else_fn));
            return;
        }
        else {
    ...
    inline decltype(auto) after(long long invocations,
        ...
        struct State {
            long long count = 0;
            long long target;
            explicit State(long long t) : target(t < 0 ? 0 : t) {}
        };

        auto& s = ::a3d::utils::flow::detail::state_for<State>(loc, invocations);

        const bool fire = (s.count++ >= s.target);

        if constexpr (std::is_void_v<R>) {
            if (fire) std::invoke(std::forward<Fn>(fn));
            return;
        }
        else {
            using T = std::decay_t<R>;
            if (fire) return std::optional<T>{ std::invoke(std::forward<Fn>(fn)) };
            return std::optional<T>{};
        }
    }
    ...
    inline bool every(std::chrono::duration<Rep, Period> interval,
        ...
        struct State {
            ...
            D interval;
            explicit State(D i) : last(Clock::now() - i), interval(i) {} // fire immediately
        };

        const D i = std::chrono::duration_cast<D>(interval);
        auto& s = ::a3d::utils::flow::detail::state_for<State>(loc, i);

        if (::a3d::utils::flow::detail::every_tick<Clock>(s.last, s.interval)) {
            std::invoke(std::forward<Fn>(fn));
            return true;
        }
        return false;
    ...
    inline decltype(auto) every_else(std::chrono::duration<Rep, Period> interval,
        ...
        const bool fire = ::a3d::utils::flow::detail::every_tick<Clock>(s.last, s.interval);
    ...
    inline bool every(long long n,
        ...
        struct State {
            ...
            explicit State(long long s) : step(::a3d::utils::flow::detail::clamp_step(s)) {}
        };

        auto& s = ::a3d::utils::flow::detail::state_for<State>(loc, n);
    ...
    inline decltype(auto) every_else(long long n,
        ...
        const bool fire = every(n, [&]{ std::invoke(std::forward<ThenFn>(then_fn)); }, loc);

        if constexpr (std::is_void_v<R1>) {
            if (!fire) std::invoke(std::forward<ElseFn>(else_fn));
            return;
        }
        else {
            if (fire) return std::invoke(std::forward<ThenFn>(then_fn));
