...
static shared_ptr<Node> PhysicsChainElement(float radius, float height);
...
shared_ptr<Node> PhysicsChainElement(float minorRadius, float majorRadius) {

	auto geometry = make_shared<Torus>(minorRadius, majorRadius, 16, 16);
	auto node = Node::GeometryNode(geometry);
//	auto shape = make_shared<PhysicsShape>(PHYSICS_SHAPE_TYPE::CONVEX_HULL, geometry);
//	auto body = make_shared<PhysicsBody>(PHYSICS_BODY_TYPE::DYNAMIC, shape);
//	node->physicsBody(body);
