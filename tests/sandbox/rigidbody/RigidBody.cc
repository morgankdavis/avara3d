...
namespace {
    ...
    vector<shared_ptr<Node>> SpawnBoxs(Node&         parent,
        ...
        const float totalLength =
            boxSize.x * static_cast<float>(sizeX) + padding * static_cast<float>(sizeX - 1);
        const float totalWidth =
            boxSize.z * static_cast<float>(sizeY) + padding * static_cast<float>(sizeY - 1);
        ...
        for (unsigned y = 0; y < sizeZ; ++y) {
            for (unsigned z = 0; z < sizeY; ++z) {
                for (unsigned x = 0; x < sizeX; ++x) {
                    ...
                    node->position({startX + static_cast<float>(x) * stepX,
