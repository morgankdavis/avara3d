...
public:
    ...
    static void dispatchHostUpdate(Application& application, Runner& runner, const Runner::UpdateInfo& info) {
...
protected:
    ...
    void drawBackground(const a3d::BackgroundPass&, const a3d::math::mat4&, const a3d::math::mat4&) override {}
...
void SimulationConfigDefaults() {
    ...
    ExpectNear(runnerConfiguration.timeScale, defaults.timeScale, "the default Runner configured time scale");
...
void SimulationConfigValidation() {
    ...
    expectConfigurationRejected(configuration, "the fixed scheduler should reject a NaN configured time scale");
...
void FirstUpdateExecutesNoSimulationSteps() {
    ...
    Expect(runner.simulationStepCount() == 0, "the first Runner update should complete no simulation steps");
    ExpectNear(runner.simulationTime(), 0.0, "the first Runner update should not advance simulation time");
    ExpectNear(node->physicsBody()->centerOfMass().x, 0.0, "the first Runner update should not advance Bullet",
               1e-6);
    ...
    Expect(stats.simulationStepsThisUpdate == 0, "the first update statistics sample should report zero steps");
    ...
    Expect(stats.dynamicBodies == 1, "a zero-step Runner update should still sample current body inventory");
    Expect(stats.primitiveShapes == 1, "a zero-step Runner update should still sample current shape inventory");
}
...
void SimulationRetainsFractionalAccumulator() {
    ...
    ExpectNear(runner.simulationTime(), 0.25, "a zero-step update should retain cumulative simulation time");

    const auto& samples = a3d::testing::RunnerTestAccess::frameStatsHistory(runner).samples();
    Expect(samples.size() == 5, "simulation scheduling should commit one statistics record per Runner update");
...
void SimulationLimitsCatchUpAndReportsDiscard() {
    ...
    ExpectNear(runner.simulationTime(), 0.25, "only completed catch-up steps should advance simulation time");
...
void StopBetweenUpdates() {
    ...
    Expect(infoAfterStop.updateIndex == infoBeforeStop.updateIndex, "stop should not advance the update index");
    ExpectNear(infoAfterStop.elapsedTime, infoBeforeStop.elapsedTime, "stop should not advance elapsed time");
...
void StopDuringHostUpdate() {
    ...
    Expect(stats.maxCatchUpSteps == 8, "the stopping host update should retain the configured catch-up limit");
    ...
    Expect(stats.simulationStepsThisUpdate == 0, "the stopping host update should commit zero-step statistics");
    ExpectNear(stats.discardedSimulationTime, 0.0,
    ...
    Expect(runner.state() == a3d::Runner::State::Stopped, "the Runner should remain stopped after hostUpdate");
...
void StopDuringInputCallbackSkipsSimulationAndRender() {
    ...
    Expect(a3d::testing::RunnerTestAccess::completedRenderFrameCount(runner) == completedRenderFramesBeforeStop,
    ...
    Expect(!a3d::testing::RunnerTestAccess::update(runner, startTime + std::chrono::milliseconds(250)),
           "a later update should remain stopped");
    Expect(result.inputCallbackCount == 1, "a stopped Runner should not invoke inputContextDidUpdate again");
...
void StopDuringSimulationStep() {
    ...
    const auto& stats = LatestFrameStats(runner);
    Expect(stats.simulationStepsThisUpdate == 1, "the stopped Runner update should report its completed step");
...
void InputAndApplicationCallbacksRunInPipelineOrder() {
    ...
    scene.willStepCallback([&](a3d::Scene& callbackScene, const a3d::Scene::StepInfo& info) {
        a3d::testing::ApplicationTestAccess::dispatchSceneWillStep(application, runner, callbackScene, info);
    });
...
void ApplicationUpdateCallbacksRunOnceAcrossCatchUpSteps() {
    ...
    Expect(inputCallbackCount == 1, "inputContextDidUpdate should not repeat for catch-up simulation steps");
...
void InputCallbackReceivesApplicationRunnerContextAndUpdateInfo() {
    ...
    Expect(callbackRunnerPtr == &runner, "inputContextDidUpdate should receive the Application-owned Runner");
    ...
    Expect(inputUpdateInfo.updateIndex == 0, "inputContextDidUpdate should receive the enclosing update index");
...
void ApplicationRunnerAccessUsesPreparedRunner() {
    ...
    ExpectNear(result.hostUpdateTimeScale, 0.5, "hostUpdate should be able to change the Runner time scale");
}
...
void SimulationCallbacksRunInOrderWithPhysics() {
    ...
    application.simulationDidAction = [&](a3d::Scene& callbackScene, const a3d::Scene::StepInfo& info) {
        Expect(&callbackScene == &scene, "sceneDidStep should receive the same Scene that completed the step");
        stages.push_back("simulation-did");
        simulationSteps.push_back(RecordStep(info));
        positionAfterPhysics = bodyNode->physicsBody()->centerOfMass().x;
        ExpectNear(runner.simulationTime(), 0.0, "Runner simulation time should not advance inside callbacks");
...
void NeitherWorldPipeline() {
    ...
    Expect(runner.simulationStepCount() == 1, "the neither-world Scene should complete one simulation step");
...
void SceneRootInvariant() {
    ...
    Expect(scene.rootNode() == originalRoot, "a rejected root replacement should preserve the original root");
...
void PhysicsInventoryPrecedesDidStep() {
    ...
    Expect(stats.primitiveShapes == 1, "the completed-step shape inventory should precede callback mutation");
...
void BulletAdvancesExactlyOncePerSimulationStep() {
    ...
    ExpectNear(node->physicsBody()->totalForce().x, 0.0, "an exact Bullet step should preserve force clearing",
               1e-6);
}
...
void SimulationPhysicsIsRepeatable() {
    ...
    for (std::size_t index = 0; index < catchUp.steps.size(); ++index) {
        ...
        ExpectStep(catchUp.steps[index], index, startTime, endTime, 0.125, "the catch-up repeatability step");
    ...
    Expect(catchUp.simulationStepCount == distributed.simulationStepCount && catchUp.simulationStepCount == 3,
...
void PausedUpdatesKeepInputCallbacksHostAndStatisticsActive() {
    ...
    application.inputAction = [&](a3d::Runner&, a3d::InputContext&, const a3d::InputContext::UpdateInfo& info) {
        stages.push_back("input-did-update");
        ++inputCallbackCount;
        inputUpdateInfo = info;
    };
    ...
    ExpectNear(inputUpdateInfo.elapsedTime, 1.0, "paused input callbacks should receive elapsed update time");
    ExpectNear(inputUpdateInfo.deltaTime, 1.0, "paused input callbacks should receive the normal update delta");
    ExpectNear(hostUpdateInfo.elapsedTime, inputUpdateInfo.elapsedTime,
    ...
    const auto& stats = LatestFrameStats(runner);
    ExpectNear(stats.simulationTimeStep, 0.125, "paused statistics should retain the configured fixed delta");
...
void PauseDuringCatchUpFinishesOnlyCurrentStep() {
    ...
    Expect(stats.simulationStepCount == 1, "pause during catch-up should report one cumulative completed step");
    ExpectNear(stats.simulationTime, 0.125, "pause during catch-up should report completed simulation time");
    ExpectNear(stats.discardedSimulationTime, 0.0,
...
void RequestedStepsUseFixedDeltaAndIgnoreTimeScale() {
    ...
    for (std::size_t index = 0; index < simulationSteps.size(); ++index) {
        ...
        const double startTime = static_cast<double>(index) * 0.125;
        const double endTime = static_cast<double>(index + 1) * 0.125;
        ExpectStep(simulationSteps[index], index, startTime, endTime, 0.125, "the requested simulation step");
    }
    ...
    Expect(stats.maxCatchUpSteps == 1, "requested-step statistics should retain the automatic catch-up limit");
...
void StopInterruptsRequestedStepsAfterCurrentStep() {
    ...
    const auto& stats = LatestFrameStats(runner);
    Expect(stats.simulationStepsThisUpdate == 1, "stop-interrupted request statistics should report one step");
    Expect(stats.simulationStepCount == 1,
